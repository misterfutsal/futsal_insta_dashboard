...
try:
    ...
    # 1. Ranking: Nimmt den jeweils letzten Eintrag JEDES Vereins
    ...
    # Rang-Zahlen passend zur Anzahl der Vereine
    ...
    # Aktuelle Werte vs. Werte von vor ca. 4 Wochen
    ...
    # Hier war der Fehler: Jetzt wird dynamisch gezählt (1 bis Anzahl Zeilen)
    ...
    with col_rank:
        ...
        st.caption(f"Stand: Letzte verfügbare Daten pro Verein")
    ...
    with col_trend:
        st.subheader("📈 Top Trends (4 Wochen)")
        st.caption(f"Vergleich: Heute vs. {closest_old_date.strftime('%d.%m.%Y')}")
    ...
    if selected_club:
        ...
        st.info("💡 Klicke oben links in der Tabelle auf einen Verein, um Details zu sehen.")
